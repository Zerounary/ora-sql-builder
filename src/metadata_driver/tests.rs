...
fn insert_uses_parameters_and_raw_sql_expressions() {
    ...
    assert_eq!(
        query.params,
        vec![json!(1), json!(37), json!(27), json!(893), json!(893), json!("默认值"), json!("名称"), json!(30)]
    );
}
...
fn update_uses_mixed_parameter_and_raw_assignments() {
    ...
    assert_eq!(query.params, vec![json!(893), json!("新名称"), json!("Y"), json!(1)]);
}
...
fn grouped_select_supports_explicit_filter_ast_and_having() {
    ...
    assert_eq!(query.params, vec![json!("零售部"), json!("批发部"), json!("Y"), json!(100)]);
}
